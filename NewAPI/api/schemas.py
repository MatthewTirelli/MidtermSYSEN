...
class TrafficWindowRow(BaseModel):
    """One row per segment for GET /traffic/window: aggregated metrics for a time window."""

    segment_id: str
    mean_flow_vph: float
    mean_speed_kmh: float | None = None
    mean_travel_time_sec: float | None = None
    vc_ratio: float | None = None
