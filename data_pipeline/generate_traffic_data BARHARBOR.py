"""
Pipeline: OSM road network -> road_segments.csv + traffic_observations.csv (raw sensor data)
for Supabase upload. BPR (speed/travel time) is applied in the API layer, not here.
Follows .cursor/rules/csv-supabase-data-strategy.mdc.
"""
...
# --- Step 7: Export CSVs (observations are raw sensor data: segment_id, timestamp, flow_vph; BPR applied in API) ---
...
def run_pipeline(use_demo: bool = False, local_osm_override: str = None):
    ...
    print("Step 7: Exporting CSVs (raw observations; BPR is applied in the API layer)...")
    base_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(base_dir)
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--input",
        type=str,
        default="bar_harbor.osm",
        help="OSM file from data_pipeline/input/ (default: bar_harbor.osm, e.g. bar_harbor.osm)",
    )
    args = p.parse_args()
    input_override = None
    if args.input:
